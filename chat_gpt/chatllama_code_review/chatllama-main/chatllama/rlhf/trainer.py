...
class ActorCritic(torch.nn.Module): # 실전코드
...
class RLTrainer:
    ...
    @beartype
    def learn(self, memories: Deque[Memory]) -> None: # learn 부분에서 update가 일어난다.
        ...
        for epoch in range(epochs):
            for i, (
                ...
                # upate critic with loss : 정답과 실제의 차이를 update
    ...
    def train(
        ...
        for eps in range(current_episode, num_episodes):
            for timestep in range(max_timesteps):
                ...
                for i in range(states.shape[0]):
                    self.conversation_log.add_conversation( # 대화로그
                        inputs[i],
                        completions[i],
