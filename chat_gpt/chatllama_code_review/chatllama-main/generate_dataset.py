from langchain import OpenAI, LLMChain, PromptTemplate # 언어를 chaining해주는 library
...
def create_conversation(human_agent: LLMChain, bot_agent: LLMChain):
    conversation = []
    chatbot_output = ""
    for i in range(CONVERSATION_LENGTH): # 10번
...
def build_agents():
    ...
    human_agent = LLMChain(
        ...
        memory=ConversationBufferWindowMemory(k=4), # 4개정도 저장
    )
